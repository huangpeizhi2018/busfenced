...
func (s *Server) Run() error {
	...
	go func() {
		...
		errchan <- s.enterEventDump(ENTER)
	}()

	//出围栏事件存储
	go func() {
		s.log.Info("eventDump EXIT startup")
		errchan <- s.enterEventDump(EXIT)
	}()

	//转发Dispatch
	go func() {
		s.log.Info("fetchDispatch service startup",
			zap.String("redis.addr", s.cf.Target.Addr), zap.String("redis.port", s.cf.Target.Port),
			zap.String("redis.point", s.cf.Source.DispatchPoint), zap.String("redis.touch", s.cf.Source.DispatchTouch),
		)
	...
	//更新Dispatch信息到Hooks
	go func() {
		s.log.Info("updateDispatch service startup")
	...
	//转发GPS
	go func() {
		s.log.Info("fetchGPS service startup",
			zap.String("redis.addr", s.cf.Source.Addr), zap.String("redis.port", s.cf.Source.Port),
			zap.String("redis.point", s.cf.Source.GPSPoint), zap.String("redis.touch", s.cf.Source.GPSTouch),
		)
		errchan <- s.fetchGPS()
	}()
	...
	//更新GPS信息到围栏处理集合
	go func() {
		s.log.Info("updateGPS service startup")
		errchan <- s.updateGPS()
	}()
	...
	if s.cf.Stats.Valid {
		go func() {
			...
			s.log.Info("stats service startup",
				zap.String("addr", s.cf.Stats.Addr),
				zap.String("port", s.cf.Stats.Port),
			)
	...
	if s.cf.AOFShrink.Valid {
		go func() {
			for {
				s.log.Info("aofshrink service startup",
				...
				s.log.Info("aofshrink service end",
...
//围栏事件转储
func (s *Server) enterEventDump(ft FenceType) error {
