...
func (g *GPS) String() string {
	return fmt.Sprintf("obuid=%s,lat=%f,lon=%f,height=%f,dir=%f,speed=%f,gpstime=%s",
		g.Obuid, g.Lat, g.Lon, g.Height, g.Dir, g.Speed, g.GPSTime.Format(time.RFC3339))
}
...
func (g *GPS) Json() string {
	...
	jstr := string(bs)
	value, _ := sjson.Set(jstr, "gpstime", g.GPSTime.Format(time.RFC3339))
